      ...
      if int(packet['MQTT'].len) > 0:
        ...
        if int(packet['MQTT'].msgtype) == 3: # PUBLISH
          ...
          try:
            ...
            double_semicolon_separated = re.match('^([0-9]+;)([a-zA-Z-]+;)([0-9a-zA-Z,]+)$', packet['MQTT'].msg_text)
            single_semicolon_separated = re.match('^([0-9]+;)([0-9a-zA-Z,-]+)$', packet['MQTT'].msg_text)
            comma_separated = re.match('^([0-9]+,)(.*)$', packet['MQTT'].msg_text)
          except AttributeError:
            double_semicolon_separated = re.match('^([0-9]+;)([a-zA-Z-]+;)([0-9a-zA-Z,]+)$', packet['MQTT'].msg)
            single_semicolon_separated = re.match('^([0-9]+;)([0-9a-zA-Z,-]+)$', packet['MQTT'].msg)
            comma_separated = re.match('^([0-9]+,)(.*)$', packet['MQTT'].msg)

          if double_semicolon_separated:
            packet_bytes['Payload']['RPCCounters'] += len(double_semicolon_separated.group(1))
            packet_bytes['Payload']['NETCONFFunctions'] += len(double_semicolon_separated.group(2))
            comma_separated = re.match('^([0-9]+,)(.*)$', double_semicolon_separated.group(3))
            if comma_separated:
              packet_bytes['Payload']['SliceNumbers'] += len(comma_separated.group(1))
              packet_bytes['Payload']['Slices'] += len(comma_separated.group(2))
            else:
              packet_bytes['Payload']['NETCONFParams'] += len(double_semicolon_separated.group(3))
          elif single_semicolon_separated:
            packet_bytes['Payload']['RPCCounters'] += len(single_semicolon_separated.group(1))
            packet_bytes['Payload']['NETCONFParams'] += len(single_semicolon_separated.group(2))
          elif comma_separated:
            packet_bytes['Payload']['SliceNumbers'] += len(comma_separated.group(1))
            packet_bytes['Payload']['Slices'] += int(packet['MQTT'].len) - topic_bytes - len(comma_separated.group(1))
          else:
            try:
              # NOTE: packet['MQTT'].msg_text is only available starting from version 3.2.0 (see https://www.wireshark.org/docs/dfref/m/mqtt.html).
              print("WARNING: MQTT payload has unhandled format", packet['MQTT'].msg_text)
            except AttributeError:
              print("WARNING: MQTT payload has unhandled format", packet['MQTT'].msg)
        else:
